...
作为 `deployments.llm-vllm` 纳入 [`controller/`](../../../controller/DESIGN.md#deployment-reconcile) 的 deployment
...
- 每个 host 最多一个 LLM serving container，identity 只由 deployment id 决定：作为控制面 deployment，容器名为
  `codespace-llm-vllm`，host 单例、不含 workspace/instance ID。不进 workspace 生命周期，不参与
...
- **模型权重不烤进镜像**：~172 GiB FP8 权重经 bind-mount 的 Hugging Face cache 在首次启动时由引擎拉取；该 cache
  即控制面 deployment 的托管数据目录 `~/codespace/deployments/llm-vllm`（config volume 用 `${DEPLOYMENT_DATA}`
  占位符引用），`purge` 清理即删该目录。
...
| --- | --- | --- | --- |
| vLLM | `ghcr.io/curoky/devspace:llm-vllm` | uv venv 从 vLLM per-commit nightly index 装 pinned nightly wheel（`${VLLM_COMMIT}` 默认 `e126687`，cu128 torch backend） | `llm` |

组装顺序对齐 sidecar：Debian slim → apt 基础包 → binman 装 standalone s6/execline 与 uv → uv 建 venv 装推理栈
→ `COPY vllm/rootfs/` → `setup-s6.sh` → s6 init。推理栈装进独立 venv `/opt/llm/venv`（`LLM_VENV`），`serve.sh`
...
**CUDA 版本（默认 cu128）**：vLLM 的默认 PyPI wheel 现已升到 CUDA 13（需 host 驱动 ≥580），但目标 8×H100 节点
...
`--extra-index-url https://download.pytorch.org/whl/${CUDA_TAG}` 解析匹配的 torch；nightly wheel 只编译
**CUDA 12.8**（无独立 cu129 nightly 变体），故 `ARG CUDA_TAG` 默认 `cu128`（H100 driver 535 跑 cu128 wheel 无碍，
避开 cu13 默认的 `libcudart.so.13` 报错）。CUDA 13 host 用 `CUDA_TAG=cu130` 覆盖。
...
相关 env，需临时改参用 `LLM_EXTRA_ARGS` 覆盖。写死项：TEP8（`--tensor-parallel-size 8 --enable-expert-parallel`）、
...
```bash
images/deployments/vllm/build.sh         # 产出 ghcr.io/curoky/devspace:llm-vllm
images/deployments/vllm/run.sh           # 在当前 GPU host 直接启动 vLLM
```

**标准运行路径由控制面 deployment 负责**：在 config 的 `deployments.llm-vllm` 声明镜像与容器块，在目标 GPU host
的 `hosts.<host>.deployments` 里选中该引擎，再于 UI 点 Deploy（或
`POST /api/deployments/llm-vllm/hosts/<host>/deploy`）。控制面按解析后 `container` 块创建容器，形态与下述要求
...
- Hugging Face cache 用 `${DEPLOYMENT_DATA}:/root/.cache/huggingface` volume 绑到托管数据根
  `~/codespace/deployments/llm-vllm`，首次启动拉取 ~172 GiB 权重到该目录，需 ≥~200 GiB 空闲空间；容器内经
...
- 新增引擎参数优先经 `serve.sh` 环境变量暴露，不写死在 s6 run 脚本；引擎命令随官方 recipe 变化时更新
  `serve.sh` 并同步本文。
- 容器运行形态（GPU/IPC、HF cache volume、网络和端口）改动必须同时更新 config 的 `deployments.llm-vllm`、
