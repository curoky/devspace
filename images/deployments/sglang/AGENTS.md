# Codespace 推理 Serving 约束（SGLang）
...
作为 `deployments.sglang` 纳入 [`controller/`](../../../controller/DESIGN.md#deployment-reconcile) 的 deployment
...
- 每个 host 最多一个推理 serving container，identity 只由 deployment id 决定：作为控制面 deployment，容器名为
  `codespace-sglang`，host 单例、不含 workspace/instance ID。不进 workspace 生命周期，不参与
...
- **模型权重不烤进镜像**：~172 GiB FP8 权重经 bind-mount 的 Hugging Face cache 在首次启动时由引擎拉取；该 cache
  即控制面 deployment 的托管数据目录 `~/codespace/deployments/sglang`（config volume 用 `${DEPLOYMENT_DATA}`
  占位符引用），`purge` 清理即删该目录。
...
| --- | --- | --- | --- |
| SGLang | `ghcr.io/curoky/devspace:deployments-sglang` | uv venv 从 PR 源码装（`${SGLANG_REF}` 默认 `pull/36497/head`，cu129 kernel） | `serve` |

组装顺序对齐 sidecar：Debian slim → apt 基础包（含 `git`）→ binman 装 standalone s6/execline 与 uv → uv 建 venv
装推理栈 → `COPY sglang/rootfs/` → `setup-s6.sh` → s6 init。推理栈装进独立 venv `/opt/sglang/venv`（`SERVE_VENV`），
...
库修复 torch；同一步还从 cu129 index 一并 `--reinstall` `torchcodec`——它是 sglang 多模态 ASR processor 的传递
依赖、非 nvidia 包故躲过 cu13 清理，但其预编译 `libtorchcodec_core*.so` 链接 `libnvrtc.so.13`（仅由被删的
`nvidia-cuda-nvrtc-cu13` 提供），不修复则 import 时 sglang 记 `OSError: libnvrtc.so.13: cannot open shared
object file` 并跳过这些 processor；重装 cu12 变体使其改链 `libnvrtc.so.12` 并拉回 `nvidia-cuda-nvrtc-cu12`。末尾
assert `torch.version.cuda` 仍为 12.x，并 `import torchcodec.decoders` 冒烟校验。venv 由此从 ~13GiB 降到 ~11GiB。
安装 RUN 末尾还 `rm -rf /root/.cache/uv` 清掉 uv 下载/构建缓存（运行期无用）；因 uv 默认把缓存文件 hardlink 进
venv，同层删缓存会破坏硬链，故设 `ENV UV_LINK_MODE=copy` 改为拷贝，保证删缓存后 venv 自包含（对齐
`images/deps/` 编译镜像做法）。
...
（含 `SERVE_MODEL`/`SERVE_HOST`/`SERVE_PORT`/`SERVE_EXTRA_ARGS`）后 `exec /opt/sglang/serve.sh`；日志写 `/var/log/serve.log`。
...
相关 env，需临时改参用 `SERVE_EXTRA_ARGS` 覆盖。写死项：TEP8（`--tp-size 8 --ep-size 8`）、
...
- `serve.sh` 针对 80 GB 写死了适配参数：`--mem-fraction-static 0.85` 给 Mamba/GDN state cache 与 activation 留
  ...
  精度），需要时经 `SERVE_EXTRA_ARGS` 加 `--kv-cache-dtype fp8_e4m3`。
- OOM 时：经 `SERVE_EXTRA_ARGS` 覆盖 `--context-length` / `--mem-fraction-static` 调小。
...
```bash
images/deployments/sglang/build.sh       # 产出 ghcr.io/curoky/devspace:deployments-sglang
images/deployments/sglang/run.sh         # 在当前 GPU host 直接启动 SGLang
```

**标准运行路径由控制面 deployment 负责**：在 config 的 `deployments.sglang` 声明镜像与容器块，在目标 GPU
host 的 `hosts.<host>.deployments` 里选中该引擎，再于 UI 点 Deploy（或
`POST /api/deployments/sglang/hosts/<host>/deploy`）。控制面按解析后 `container` 块创建容器，形态与下述要求
...
- Hugging Face cache 用 `${DEPLOYMENT_DATA}:/root/.cache/huggingface` volume 绑到托管数据根
  `~/codespace/deployments/sglang`，首次启动拉取 ~172 GiB 权重到该目录，需 ≥~200 GiB 空闲空间；容器内经
...
- 新增引擎参数优先经 `serve.sh` 环境变量暴露，不写死在 s6 run 脚本；引擎命令随官方 recipe 变化时更新
  `serve.sh` 并同步本文。
- 容器运行形态（GPU/IPC、HF cache volume、网络和端口）改动必须同时更新 config 的 `deployments.sglang`、
