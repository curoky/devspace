...
| 场景 | 路径 / 用法 |
| --- | --- |
| Python | 优先 `/opt/sb/bin/uv run <cmd>`；`ruff` 在 `/opt/sb/bin/ruff` |
...
| pnpm | `/opt/sb/bin/pnpm`、`/opt/sb/bin/pnpx` |
| Go | `/home/x/.nix-profile/bin/go`、`gofmt` |
| Go tools | `/opt/sb/store/<name>/bin/<name>`，如 `gopls`、`golangci-lint`、`dlv` |
...
| C/C++ | `/home/x/.nix-profile/bin/gcc`、`g++`；`clang-format` 在 `/opt/sb/store/clang-tools-*/bin/clang-format` |
| 构建工具 | `/opt/sb/bin/cmake`、`ninja`、`bazel`、`protoc`、`task` |
| Shell | `/opt/sb/bin/shfmt`、`shellcheck` |
| 常用 CLI | `/opt/sb/bin` 下有 `git`、`gh`、`rg`、`fd`、`jq`、`curl`、`ssh`、`rsync`、`zip` 等 |
