...
CUDA userspace runtime 由 host 的 NVIDIA Container Toolkit 在运行期提供，推理 wheel 自带其余 CUDA runtime 库。
**SGLang 例外——需完整 CUDA Toolkit（nvcc + 头文件）**：SGLang 的 FP8 路径依赖 `sgl-deep-gemm`，其 kernel 在
...
*runtime* 库，二者都不含 nvcc/dev 头。因此 SGLang 镜像用 multi-stage：从 NVIDIA 官方 CUDA devel 镜像
...
镜像构建在仓库根手动执行（发布由 `.github/workflows/` 管理），各引擎脚本在自己子目录：
