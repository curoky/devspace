...
# Launch the single host-level vLLM Qwen3.8-Flash-Next-FP8 serving container on a
# single 8x H100 node. Mirrors the sidecar launcher shape: fixed container name,
...
# it by default. If the load OOMs, lower MAX_MODEL_LEN or set
# VLLM_PLE_CPU_OFFLOAD=1 to move the 51B N-gram table to host RAM.
...
echo "llm '${name}' (vllm) starting on http://127.0.0.1:${port}."
