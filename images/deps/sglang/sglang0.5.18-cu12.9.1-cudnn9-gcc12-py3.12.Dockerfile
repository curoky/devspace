...
# `import sglang` 的可运行镜像（无 s6、无 serving entrypoint）。与 images/deployments/ 的
# serving 镜像刻意区分。
#
...
# images/deployments/sglang/Dockerfile 保持一致：卸载 13.x/_cu13 的 nvidia 包（保留
...
RUN set -eux; \
  ...
  ${DEPS_UV} --force-reinstall sgl-deep-gemm --no-deps \
    --index-url "https://docs.sglang.ai/whl/${CUDA_TAG}/"; \
  CU13_PKGS="$(ls -d ${DEPS_VENV}/lib/python3.12/site-packages/*.dist-info \
    | sed 's#.*/##;s/.dist-info//' \
  ...
  "${DEPS_VENV}/bin/python" -c "import torch; assert torch.version.cuda.startswith('12'), torch.version.cuda"
