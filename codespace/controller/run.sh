...
log_file="${repo_root}/codespace-controller.log"
uv_bin="$(command -v uv)"
readonly uv_bin
readonly process_pattern='[p]ython[^ ]* -m codespace[.]controller'
...
nohup "${uv_bin}" run --directory "${repo_root}" python -m codespace.controller \
