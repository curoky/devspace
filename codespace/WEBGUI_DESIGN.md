...
- 支持 GitHub / GitLab provider，token 只从环境变量读取。
...
Local FastAPI Web GUI
  ...
  │ 使用本地 Git provider token 注册 / 吊销 deploy key
  │
...
| 字段 | 必填 | 说明 |
| --- | --- | --- |
| `token_env` | 否 | GitHub token 环境变量名，默认 `GITHUB_TOKEN`。 |

#### `gitlab`

| 字段 | 必填 | 说明 |
| --- | --- | --- |
| `token_env` | 否 | GitLab token 环境变量名，默认 `GITLAB_TOKEN`。 |
| `api_url` | 否 | GitLab API 地址，默认 `https://gitlab.com`。 |
| `ssh_host` | 否 | GitLab SSH host，默认 `gitlab.com`。 |
...
| `git_ssh_host` | 否 | clone 使用的 Git SSH host；不填取 provider 默认值。 |
...
- `token_env` 必须是环境变量名；配置文件不接受 token 明文。
...
├──────────────────────────────────────────────────────────────────────────────┤
│ Runtime strip: token readiness · last updated · auto refresh · agent chips   │
├──────────────────────────────────────────────────────────────────────────────┤
│ Templates                                                                    │
...
- agent 状态和 token 状态常驻，避免创建失败时用户不知道原因。
...
defaults 填充，用户提交前仍可调整。表单还支持填写非敏感环境变量，每行 `KEY=VALUE`，提交前由
前端解析为 `env` 对象。
...
| `repo` | 目标 repo。 |
| `provider` | Git provider。 |
| `git_ssh_host` | clone host。 |
| `template` | template id。 |
| `instance` | instance name。 |
| `status` | `queued` / `running` / `succeeded` / `failed`。 |
| `stage` | 当前阶段文本。 |
| `codespace` | 成功后返回的 codespace。 |
| `error` | 失败原因。 |
...
```json
{
  ...
  "github": {"token_env": "GITHUB_TOKEN", "has_token": true},
  "gitlab": {
    "token_env": "GITLAB_TOKEN",
    "api_url": "https://gitlab.com",
    "ssh_host": "gitlab.com",
...
### 11.5 `DELETE /api/operations`

清理已完成 operation，保留 queued/running。

### 11.6 `DELETE /api/agents/{agent_id}/codespaces/{codespace_id}`
...
webui/src/utils.ts    # alias、颜色、格式化、分组工具、环境变量文本解析
```
...
- token 只从环境变量读取。
- `/api/config` 只返回 env 名称和 `has_token`，不返回 token 值。
- agent 不接收 token。
