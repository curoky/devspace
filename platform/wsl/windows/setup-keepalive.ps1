<#
.SYNOPSIS
    Fallback keep-alive for the codespace WSL distribution via a Scheduled Task.

.DESCRIPTION
    Use this when windows/wslconfig.sample (instanceIdleTimeout=-1) does not keep
    the distribution running -- e.g. older WSL builds that ignore that key, or
    the WSL 2.6.x regression where even systemd services fail to keep the distribution
    alive.
...
.PARAMETER Distro
    WSL distribution name. Defaults to "codespace".

.EXAMPLE
    # Run in an elevated PowerShell:
    .\setup-keepalive.ps1
    .\setup-keepalive.ps1 -Distro codespace
#>

param(
    [string]$Distro = 'codespace'
)
