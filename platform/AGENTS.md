...
`platform/` 拥有 OCI image、macOS Host 配置和 WSL 资产，不包含 control plane
业务代码。
...
- macOS installer 必须幂等；WSL rootfs 不承担 Windows Host 配置。

跨平台约束变化时必须覆盖所有消费该约束的 image 与 macOS installer。
