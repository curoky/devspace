...
# the init's PATH.
#
# The image must copy the Workspace-owned /etc/s6/skel and its own s6-rc service
# definitions before invoking this script. The script compiles the database and
# generates the container init from that skeleton.
...
#     with the container manager, rc.init gets the configured default runlevel).
...
#     scripts) from there, so it must equal the generated dir.
rm -rf /etc/s6/init
