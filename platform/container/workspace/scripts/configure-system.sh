...
# User provisioning and system configuration. Static files are copied by the
# Dockerfile; this script performs operations that cannot be represented by the
# rootfs trees.
...
# mount FUSE: init-workspace runs gocryptfs as `x` (so ciphertext files land
