"""Shared helpers for the out-of-band maintenance CLIs.

The three ``tools`` commands (secret sync, workspace and deploy-key cleanup) all
fan work out across hosts or repositories concurrently and render the same
table / warning / error report. Those mechanics live here so each command only
describes what to collect, show and apply.
"""
...
def render_table(console: Console, columns: list[dict[str, object]], rows: Iterable[tuple]) -> None:
